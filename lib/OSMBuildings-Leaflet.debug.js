...
(function(global) {
    ...
    var requestAnimFrame = (global.requestAnimationFrame && !IS_IOS && !IS_MSIE) ? global.requestAnimationFrame : function(callback) {
        callback();
    };
    //****** file: Color.debug.js ******
    ...
    function simplifyPolygon(buffer) {
        ...
        while (last) {
            ...
            for (i = first + 1; i < last; i++) {
                sqDist = getSquareSegmentDistance(buffer[i * 2], buffer[i * 2 + 1], buffer[first * 2], buffer[first * 2 + 1], buffer[last * 2], buffer[last * 2 + 1]);
                if (sqDist > maxSqDist) {
                    index = i;
                    maxSqDist = sqDist;
                }
    ...
    function pixelToGeo(x, y) {
        ...
        /*  x /= MAP_SIZE;
          y /= MAP_SIZE;*/
        var coord = crs.pointToLatLng(L.point(x, y), ZOOM);
        res[LON] = coord.lng;
        res[LAT] = coord.lat;
    ...
    function geoToPixel(lat, lon) {
        ...
        var point = crs.latLngToPoint(L.latLng(lat, lon), ZOOM);
        return {
            x: point.x,
            y: point.y
        };
    ...
    var Data = {
        ...
        addRenderItems: function(datas, allAreNew) {
            var item, scaledItem, id;
            for (var i = 0, il = datas.length; i < il; i++) {
                item = datas[i];
                item.id = id = item.id || [item.footprint[0], item.footprint[1], item.height, item.minHeight].join(',');
        ...
        set: function(geojson) {
            this.isStatic = true;
            this.resetItems();
            this._staticData = GeoJSON.read(geojson);
        ...
        add: function(geojson) {
            ...
            this.resetItems();
            if (this._staticData) {
                this._staticData = this._staticData.concat(GeoJSON.read(geojson));
            } else {
                this._staticData = GeoJSON.read(geojson);
            }
        ...
        /**
         * [geojsonMerge merge([geojson, geojson2...])]
         * @param  {[type]} inputs [description]
         * @return {[type]}        [description]
         */
        geojsonMerge: function(inputs) {
            var featuretypes = {
                Point: 'geometry',
                MultiPoint: 'geometry',
                LineString: 'geometry',
             ...
             */
            var normalize = function(gj) {
                if (!gj || !gj.type) return null;
                var type = featuretypes[gj.type];
                if (!type) return null;
                if (type === 'geometry') {
                    return {
        ...
        update: function() {
            ...
            function callback(json) {
                scope.addRenderItems(GeoJSON.read(json));
            }
            for (y = minY; y <= maxY; y++) {
                for (x = minX; x <= maxX; x++) {
    ...
    var Buildings = {
        ...
        renderitem: function(item, context) {
            var wallColor, altColor, roofColor, h, mh;
            ...
            switch (item.shape) {
                ...
                case 'sphere':
                    ...
                    break;
                case 'pyramid':
                    Pyramid.draw(context, item.footprint, item.center, h, mh, wallColor, altColor);
                    break;
                default:
                    Block.draw(context, item.footprint, item.holes, h, mh, wallColor, altColor, roofColor);
            }
            switch (item.roofShape) {
                ...
                case 'dome':
                    ...
                    break;
                case 'pyramid':
                    Pyramid.draw(context, item.footprint, item.center, h + item.roofHeight, h, roofColor, Color.parse(roofColor).lightness(0.9));
                    break;
            }
        },
        render: function() {
    ...
    proto.onMove = function(e) {
        var off = this.getOffset();
        moveCam({
            x: this.offset.x - off.x,
            y: this.offset.y - off.y
        });
    };
    proto.onMoveEnd = function(e) {
        ...
        setSize({
            width: map._size.x,
            height: map._size.y
        }); // in case this is triggered by resize
        setOrigin({
            x: po.x - off.x,
            y: po.y - off.y
        });
     ...
     */
    proto.highlight = function(id, style) {
        if (!id&&this.highlightid!=id) {
           Data.update();
           console.log("out");
        }
        if (id&&this.highlightid!=id) {
          this.setItemStyle(id, style);
          console.log("in");
        }
        this.highlightid=id;
    ...
    proto.setItemStyle = function(id, style) {
        var item = Data.getItem(id);
       
        style = style || {};
        var color;
        if ((color = style.color || style.wallColor) && item) {
            item.wallColor = '' + Color.parse(color).alpha(ZOOM_FACTOR);
            item.altColor = '' + Color.parse(color).alpha(ZOOM_FACTOR).lightness(0.8);
            item.roofColor = '' + Color.parse(color).alpha(ZOOM_FACTOR).lightness(1.2);
            if (style.roofColor) {
                item.roofColor = '' + Color.parse(style.roofColor).alpha(ZOOM_FACTOR);
            }
             Buildings.renderitem(item,Buildings.context);
           // Buildings.render();
        }
    };
    ...
    proto.mousemove = function(handler) {
        onMouseMove = function(payload) {
            return handler(payload);
        };
        return this;
    };
