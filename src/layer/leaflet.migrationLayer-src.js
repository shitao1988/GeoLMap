(function (window) {
    ...
    L.PulseMarkLayer = L.Class.extend({
        options: {
            map: {},
            data: {},
            pulseRadius: 25,
        ...
        _init: function () {
            var container = L.DomUtil.create('div', 'leaflet-MMLayer-container');
            container.style.position = 'absolute';
            container.style.width = this._map.getSize().x + "px";
            container.style.height = this._map.getSize().y + "px";
        ...
        _convertData: function () {
            ...
            if (this._data && bounds) {
                var data = arrayUtils.map(this._data, function (d) {
                    var pixel = this._map.latLngToContainerPoint(d.latlng);
                    var d1 = {};
                    L.extend(d1, d);
                    d1.x = pixel.x;
                    d1.y = pixel.y;
        ...
        _draw: function () {
            var bounds = this._map.getBounds();
            if (bounds && this.pulsemark.playAnimation) {
                this._resize();
                this._transform();
                var data = this._convertData();
                this.pulsemark.updateData(data);
                this.pulsemark.start(this.canvas);
            }
        },
        _transform: function () {
            var bounds = this._map.getBounds();
        ...
        pause: function () {
            this.pulsemark.pause();
        },
        destroy: function () {
            this.pulsemark.clear();
            //移除dom
            this.container.parentNode.removeChild(this.container);
            //移除事件监听
            this._map.clearAllEventListeners();
    ...
    L.pulseMarkLayer = function (options) {
